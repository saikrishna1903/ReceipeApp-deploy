...
public class ReceipesController {
	...
	public String page(Details d) {
		if(d.getUsername().equals("sai") && d.getPassword().equals("Krishna11")) {
			z=true;
	...
	public ModelAndView getall1(String search) {
		logger.info("entererd into Home page ");
		ModelAndView mv=new ModelAndView();
		List<Receipes> P= ser.findByName(search);
		mv.addObject("z",z);
		mv.addObject("P",P);
		mv.addObject("F",search);
